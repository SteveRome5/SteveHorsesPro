...
def parse_decimal(s):
    ...
    m = re.fullmatch(r'(\d+)\s*[/\-:]\s*(\d+)', t)     # 5/2, 3-1, 7:2
    if m:
...
def program_no(e):
    for k in ("program_number","programNumber","number","saddle_number","saddleNumber",
              "horse_number","horseNumber","saddle","saddleNo","post_position","postPosition","pp","draw","gate"):
        v = e.get(k)
        if v not in (None,""):
            return str(v).strip()
    return "?"
...
# probability blend; weights ML if present, else uniform
def probs(entries):
    n = max(1,len(entries))
    uni = 1.0/n
    p_ml = []; have = 0
    ...
    w = 0.70 * (have/n)                     # only weight ML when coverage exists
    ...
    s = sum(out)
    return [x/s for x in out], p_ml  # also return raw ML imps for tie‑breaks
...
def main():
    ...
    for tname, mid, tid in chosen:
        ...
        for idx, r in enumerate(races, 1):
            ...
            # choose ONE top target per race using robust tie‑break:
            # 1) highest model p, 2) higher ML implied prob, 3) horse name alpha
            def key(i):
                ml = ml_imps[i] or 0.0
                nm = horse_name(field[i]).lower()
                return (-ps[i], -ml, nm)
            best_i = min(range(len(field)), key=key)
    ...
    rows.sort(key=lambda x: (str(x["track"]), int(x["race"]) if str(x["race"]).isdigit() else 999, -x["p"], x["horse"]))
