...
# PF-35 Mach++ v3.8-pro (WHY fixed + anti-flat + robust RID w/ synthetic fallback)
...
def why_feature_pack(track: str, rc: dict, runners: list[dict]):
    ...
    why=[]; tips=[]
    for i in range(n):
        why.append(f"SpeedForm {_arrow(sf_pct[i])} ({sf_pct[i]} pct), "
                   f"ClassΔ {_arrow(cls_pct[i])} ({cls_pct[i]} pct), "
                   f"Bias {_arrow(bia_pct[i])} ({bia_pct[i]} pct)")
        tips.append(f"SpeedForm {sf_z[i]:+0.2f}σ • ClassΔ {cls_z[i]:+0.2f}σ • Bias {bia_z[i]:+0.2f}σ")
    return why, tips
...
.bet{{background:#eef9f0}} .sub{{color:#555}}
    ...
    parts.append(
        f"<p class='small'>Scratches — auto races: {scr_summary.get('auto_races',0)}, "
        f"manual races: {scr_summary.get('manual_races',0)} • Daily cap: ${int(daily_cap_amt):,}</p>"
    )
    ...
    if scr_details:
        ...
        for row in sorted(scr_details, key=lambda x: (x['track'].lower(), x['race'])):
            ...
            parts.append(
                f"<tr><td>{_html.escape(row['track'])}</td>"
                f"<td>{row['race']}</td>"
                f"<td>{(row.get('source') or '—').upper()}</td>"
                f"<td class='mono'>{progs}</td>"
                f"<td class='mono'>{row.get('removed','—')}</td></tr>"
            )
    ...
    for track, races in cards.items():
        for idx_race, rc in enumerate(races, 1):
            ...
            # Robust race-id extraction + guard  (UPDATED + synthetic fallback)
            rid_raw = g(
                rc,
                ...
                for k, v in rc.items():
                    ...
                    if "id" in ks and any(tok in ks for tok in ("race", "event", "uuid", "key", "uid", "_id")):
                        if v not in (None, "", "None"):
                            rid_raw = v
                            _log(f"RID fallback picked key={k} track={track} race={rno}")
                            break
            ...
            if not rid:
                ...
                _log(f"NO vendor RID for track={track} race={rno} -> synthetic rid={rid} (willpays may be unavailable)")
            else:
                _log(f"RID ok track={track} race={rno} rid={rid}")
            ...
            # Live/aux fetches — only call APIs when we truly have an id that looks like vendor id
            cond = {"cond": "", "takeout": None}
            oh = {}
            wp = {"impl": {}, "win_pool": None}
            vendor_rid_like = "|" not in rid  # synthetic has '|'
            if vendor_rid_like:
                try:
                    c = fetch_condition(rid)
                    if c: cond = c
                except Exception as e:
                    _log(f"condition fail rid={rid}: {e}")
                try:
                    h = fetch_odds_history(rid)
                    if h: oh = h
                except Exception as e:
                    _log(f"odds fail rid={rid}: {e}")
                try:
                    w = fetch_willpays(rid)
                    if w: wp = w
                except Exception as e:
                    _log(f"willpays fail rid={rid}: {e}")

                try:
                    oh_n = (len(oh) if hasattr(oh, "__len__") else (oh.get("points") and len(oh.get("points")))) or 0
                except Exception:
                    oh_n = 0
                try:
                    wp_impl = (wp or {}).get("impl") or {}
                    wp_n = len(wp_impl) if hasattr(wp_impl, "__len__") else 0
                except Exception:
                    wp_n = 0
                _log(f"FETCH track={track} race={rno} rid={rid} got: odds_points={oh_n} willpays_impl={wp_n} takeout={cond.get('takeout')}")
            else:
                _log(f"synthetic RID in use, skipping live calls for track={track} race={rno}")
            ...
            # ---------- Race table ----------
            parts.append(f"<h2>{_html.escape(track)} — Race {rno}</h2>")
            if cond and (cond.get('cond') or cond.get('takeout') is not None):
                co = cond.get('cond') or "—"
                to = cond.get('takeout')
                to_str = (f"{to:.0%}" if isinstance(to, (int, float)) else "—")
                parts.append(f"<p class='small'>Condition: {_html.escape(str(co))} • Win takeout: {to_str}</p>")
            ...
            for row in sorted(enriched, key=lambda x: -x["p_final"]):
                ...
                market_pct = (f"{100.0*(imp or 0.0):.1f}%" if imp is not None else "—")
                edge_str = (f"{100.0*edge:.1f} pp" if edge is not None else "—")

                parts.append(
                    "<tr style='{bg}'>".format(bg=edge_color(pF, dec)) +
                    f"<td class='mono'>{_html.escape(row['num'])}</td>"
                    f"<td>{_html.escape(row['name'])}"
                    f"<div class='small sub'>{_html.escape(row['tags'])} &nbsp;•&nbsp; {_html.escape(row['mini'])}</div>"
                    f"<div class='small sub'>{_html.escape(row['why'])}</div>"
                    f"</td>"
                    ...
                    f"<td>{_html.escape(row['flags'])}</td>"
                    f"<td class='mono'>{('$'+format(int(round(row['bet'])),',d')) if (row['bet'] and row['bet']>0) else '—'}</td>"
                    "</tr>"
                )
            parts.append("</tbody></table>")
    ...
    def render_board(title, board):
        ...
        out.append("<table><thead><tr>"
                   ...
                   "<th class='mono'>Bet</th><th>Flags</th></tr></thead><tbody>")
        ...
        for b in sorted(board, key=keyer):
            market_pct = (f"{100.0*(b['imp'] or 0.0):.1f}%" if b.get("imp") is not None else "—")
            edge_str = (f"{100.0*((b['edge'] or 0.0)):.1f} pp" if b.get("edge") is not None else "—")
            out.append(
                "<tr>" +
                f"<td>{_html.escape(b['track'])}</td>"
                f"<td class='mono'>{b['race']}</td>"
                f"<td class='mono'>{_html.escape(b['num'])}</td>"
                f"<td>{_html.escape(b['name'])}</td>"
                f"<td class='mono'>{100.0*b['p']:0.2f}%</td>"
                f"<td class='mono'>{market_pct}</td>"
                f"<td class='mono'>{edge_str}</td>"
                f"<td class='mono'>{odds_formats(b['fair'])}</td>"
                f"<td class='mono'>{odds_formats(b['minp'])}</td>"
                f"<td class='mono'>{odds_formats(b['market'])}</td>"
                f"<td class='mono'>{('$'+format(int(round(b['bet'])),',d')) if (b['bet'] and b['bet']>0) else '—'}</td>"
...
if __name__ == "__main__":
    try:
        ...
        log(f"[run] {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  starting steve_horses_pro.py")
        ...
        try:
            ...
            log(f"Tracks: {n_tracks}  Races: {n_races}")
        ...
        log(f"[FATAL] build report failed: {e}")
        try:
            last = sorted(OUT_DIR.glob("*_horses_targets+full.html"))[-1]
            log(f"[fallback] Last good report: {last}")
            # macOS 'open' can be done by your launcher if desired; we do nothing here.
        except Exception:
            pass