#!/usr/bin/env python3
# pro_hotfix_flex.py — robust hotfix for WHY + separation in steve_horses_pro.py
import re
...
BACK  = BASE / f"steve_horses_pro.py.BAK_{datetime.now().strftime('%Y%m%d-%H%M%S')}"

src = PRO.read_text(encoding="utf-8")

def replace_between(start_hdr: str, end_hdr: str, replacement: str):
    global src
    p = re.compile(re.escape(start_hdr) + r"(.*?)(?=" + re.escape(end_hdr) + r")", re.S)
    if not p.search(src):
        raise RuntimeError(f"Could not locate section starting at: {start_hdr}")
    src = p.sub(start_hdr + replacement + "\n", src)

def replace_function(func_name: str, end_after_name: str, replacement: str):
    """Replace def <func_name>(...) up to (but not including) def <end_after_name>(...)"""
    global src
    p = re.compile(
        r"(def\s+" + re.escape(func_name) + r"\s*\([^)]*\)\s*:\s*)(.*?)(?=def\s+" + re.escape(end_after_name) + r"\s*\()",
        re.S
    )
    m = p.search(src)
    if not m:
        raise RuntimeError(f"Could not locate function: {func_name}")
    src = p.sub(replacement + "\n\ndef " + end_after_name + "(", src, count=1)

# ---------- Blocks ----------
WHY_START = "# ---------------- WHY (SpeedForm / ClassΔ / Bias) ----------------"
HTML_START = "# ---------------- HTML helpers ----------------"
...
def why_feature_pack(track: str, rc: dict, runners: list[dict]):
    # Use trained pars so text is bucket-aware; never flat
    ...
    # Derived vs par (same shaping TRAIN uses)
...
PACE_START = "# ---------------- Pace & handcrafted backup ----------------"
MODEL_PROBS_START = "# ---------------- Model probabilities ----------------"
...
def handcrafted_scores(track, rc, runners, extras=None):
    ...
    for i,r in enumerate(runners):
        ...
        s+=shape_adj[i]
        seed=f"{track}|{g(rc,'race_number','race','number','raceNo') or 0}|{prg_num(r)}|{horse_name(r)}"
...
def probabilities_from_model_only(track, rc, runners, extras=None):
    ...
    if ok and ps:
        ...
        # anti-flat: if too equal, separate using handcrafted prior
        rng = (max(ps)-min(ps)) if ps else 0.0
        if rng < 0.04:
            ps = anti_flat_separation(track, rc, runners, ps, extras)
        return ps
    # fallback when no bucket model
    zs = handcrafted_scores(track, rc, runners, extras=extras)
...
# ---------- Do replacements ----------
replace_between(WHY_START, HTML_START, "\n" + WHY_BLOCK + "\n")
replace_between(PACE_START, MODEL_PROBS_START, "\n" + PACE_BLOCK + "\n")
replace_function("probabilities_from_model_only", "blend_with_market_if_present", PROB_FUNC)

# Tag version so you can confirm in logs
src = src.replace('VERSION = "PF-35 Mach++ v3.8-pro"', 'VERSION = "PF-35 Mach++ v3.8-pro (WHY+Sep hotfix)"')

# Write backup + new file
BACK.write_text(PRO.read_text(encoding="utf-8"), encoding="utf-8")
PRO.write_text(src, encoding="utf-8")
print("[hotfix] applied OK ->", PRO.resolve())
print("[hotfix] backup      ->", BACK.resolve())