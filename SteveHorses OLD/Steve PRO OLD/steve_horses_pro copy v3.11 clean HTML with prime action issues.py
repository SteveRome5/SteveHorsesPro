...
# PF-35 Mach++ v3.11-pro-stable
# (WHY fixed + anti-flat + robust RID w/ synthetic fallback + Train wiring +
#  PRO confidence gates + dutched staking + daily exposure cap + full PRIME/ACTION boards)
...
# ---------------- Paths & version ----------------
VERSION = "PF-35 Mach++ v3.11-pro-stable"
HOME = Path.home()
...
def _field_adjusted_win_floors(field_size: int|None) -> Tuple[float, float]:
    """
    Field-aware Win% floors for PRIME and ACTION.

    Env:
      EDGE_WIN_PCT_FLOOR       base PRIME floor (default 0.20)
      ACTION_PCT_FLOOR         base ACTION floor (default 0.145)
      MIN_WIN_FLOOR_PRIME      lower bound for PRIME (default 0.12)
      MIN_WIN_FLOOR_ACTION     lower bound for ACTION (default 0.09)
      WIN_FLOOR_SLOPE          easing per runner beyond 7 (default 0.01)
    """
    n = int(field_size or 8)
    base_prime  = float(os.getenv("EDGE_WIN_PCT_FLOOR",  "0.20"))
    base_action = float(os.getenv("ACTION_PCT_FLOOR",    "0.145"))
    slope       = float(os.getenv("WIN_FLOOR_SLOPE",     "0.01"))
    p_min       = float(os.getenv("MIN_WIN_FLOOR_PRIME", "0.12"))
    a_min       = float(os.getenv("MIN_WIN_FLOOR_ACTION","0.09"))

    ease = max(0, n - 7) * max(0.0, slope)   # easing kicks in for fields > 7
    prime  = max(p_min,  base_prime  - ease)
    action = max(a_min, base_action - ease)
    return prime, action
...
def dutch_overlays(enriched, bankroll, field_size, late_slope_max, odds_var_mean, m2p,
    ...
    Two-lane PRIME eligibility:
      Lane A (prob-led):   p >= field-aware floor AND edge_pp >= EDGE_PP_MIN_PRIME
      Lane B (overlay-led): p >= LANE_B_MIN_P AND edge_pp >= LANE_B_MIN_EDGE_PP AND m2p <= LANE_B_MAX_MTP AND HIGH confidence
    """
    ...
    # field-adjusted Win% floors (Lane A uses floor_prime)
    ...
    for i, it in enumerate(enriched or []):
        ...
        # confidence (needed for either lane in PRO mode)
        if PRO_ON:
        ...
        # ACTION eligibility (field-aware)
...
def build_report(cards, iso_date, scr_summary, auto_summary, scr_details=None):
    ...
    def _sanitize_dec(dec: Optional[float]) -> Optional[float]:
        """
        Treat extreme / sentinel prices (e.g., ~99/1 sent as 100.0) as missing.
        Keep only plausible market quotes; leave 'None' if not usable.
        """
        try:
            if dec is None:
                return None
            ...
            if not (dec > 1.0):
                return None
            # Anything 50.0+ (≈ 49/1 or longer) is assumed to be a sentinel/missing price.
            if dec >= 50.0:
                return None
            return dec
        except Exception:
...
.badge.pro{{background:#eaffea}} .badge.train{{background:#e6f4ff}}
    ...
    for track, races in cards.items():
        ...
        for idx_race, rc in enumerate(races, 1):
            ...
            if not rid:
                ...
                log("NO vendor RID for track={} race={} -> synthetic rid={}".format(track, rno, rid))
            else:
            ...
            if vendor_rid_like:
                try: c = fetch_condition(rid);  cond = c or cond
                except Exception as e: log("condition fail rid={}: {}".format(rid, e))
                try: h = fetch_odds_history(rid); oh = h or oh
                except Exception as e: log("odds fail rid={}: {}".format(rid, e))
                try: w = fetch_willpays(rid); wp = w or wp
                except Exception as e: log("willpays fail rid={}: {}".format(rid, e))

            sect = {"pressure":0.0,"meltdown":0.0}  # placeholder

            # Market vectors (SANITIZED)
            market = []
            market_probs = []
            for r in runners:
                pr = prg_num(r)
                mkt_live = _sanitize_dec(live_decimal(r))

                # Willpays can overwrite/soft-cap. Convert implied prob -> dec and sanitize.
                implied = wp.get("impl", {}).get(pr)
                mkt_wp = None
                if implied and 0.0 < implied < 1.0:
                    try:
                        mkt_wp = _sanitize_dec(1.0 / float(max(0.01, min(0.99, implied))))
                    except Exception:
                        mkt_wp = None

                # Choose a usable market:
                # - If only one exists, use it.
                # - If both exist, take the more conservative (min) price.
                mkt = mkt_live
                if mkt is None:
                    mkt = mkt_wp
                elif mkt_wp is not None:
                    mkt = min(mkt, mkt_wp)
            ...
            for i, (r, pM, pF, dec) in enumerate(zip(runners, p_model, p_final, market)):
                ...
                enriched.append({
                    ...
                    "fair": fair,
                    "minp": minp,
                    "market": dec,          # already sanitized (or None)
                    "imp": imp,
                    "edge": None,
                    "bet": bet_amt,
                    "board": "",
            ...
            # Staking (PRIME dutch) — respects Train fixed bets
            flags_out = {}
            ...
            # PRIME Board (top 3 by bet desc, tie-break by p_final)
            ...
            # ACTION Board (limit 3 per race) — field-aware floors
            added_for_race = 0
            ...
            for row in sorted(enriched, key=lambda x: (-x["p_final"], x["fair"]))[:5]:
                ...
                # If Market% missing, still allow based on p threshold (Edge shows "—")
                if imp is None:
                    if p >= floor_action:
                        action_board.append({
                            "track": track, "race": rno, "num": row["num"], "name": row["name"],
    ...
    # ---------- De-duplicate boards ----------
    # Keep best entry per (track, race, num).
    def _k(b):
        return (b["track"].lower(), str(b["race"]), str(b["num"]))

    # PRIME: keep higher Bet, then higher Win%
    prime_best = {}
    for b in prime_board:
        k = _k(b)
        prev = prime_best.get(k)
        if (not prev) or ((b.get("bet", 0.0) > prev.get("bet", 0.0)) or
                          (b.get("bet", 0.0) == prev.get("bet", 0.0) and b.get("p", 0.0) > prev.get("p", 0.0))):
            prime_best[k] = b
    prime_board = list(prime_best.values())

    # ACTION: optionally exclude anything already on PRIME (default ON);
    # then keep higher Win%, then higher Edge
    exclude_overlap = os.getenv("ACTION_EXCLUDE_PRIME", "1") == "1"
    prime_keys = set(prime_best.keys())
    action_best = {}
    for a in action_board:
        k = _k(a)
        if exclude_overlap and k in prime_keys:
            continue
        prev = action_best.get(k)
        if (not prev) or (a.get("p", 0.0) > prev.get("p", 0.0) or
                          (a.get("p", 0.0) == prev.get("p", 0.0) and (a.get("edge") or 0.0) > (prev.get("edge") or 0.0))):
            action_best[k] = a
    action_board = list(action_best.values())
